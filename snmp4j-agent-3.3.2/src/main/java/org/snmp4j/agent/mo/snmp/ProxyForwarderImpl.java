/*_############################################################################
  _## 
  _##  SNMP4J-Agent 3 - ProxyForwarderImpl.java  
  _## 
...
public class ProxyForwarderImpl implements ProxyForwarder {
     ...
     *
     * @return <code>true</code> if the request has been forwarded,
     * <code>false</code> otherwise.
     */
    public <A extends Address> boolean forward(ProxyForwardRequest<A> request) {
        ...
        if (SnmpRequest.getViewType(pduType) == VACM.VIEW_NOTIFY) {
            ...
            ResponseEvent<A> resp = singleForward(request);
    ...
    protected List<SnmpProxyRow> getMatches(ProxyForwardRequest<?> request) {
        List<SnmpProxyRow> matches =
                proxyMIB.getProxyRows(request.getProxyType(),
                        request.getContextEngineID(),
                        request.getContext());
        for (Iterator<SnmpProxyRow> it = matches.iterator(); it.hasNext(); ) {
            OctetString paramIn = it.next().getSnmpProxyTargetParamsIn();
    ...
    protected <A extends Address> ResponseEvent<A> singleForward(ProxyForwardRequest<A> request) {
        List<SnmpProxyRow> matches = getMatches(request);
        ...
        Target<A> target = (Target<A>)targetMIB.getTarget(outParam, request.getContextEngineID(), request.getContext());
        if (target == null) {
            if (logger.isInfoEnabled()) {
     ...
     */
    protected void proxyForwardTranslation(ProxyForwardRequest<?> request, PDU source, PDU target)
            throws IllegalArgumentException {
        target.clear();
    ...
    protected <A extends Address> boolean multipleForward(ProxyForwardRequest<A> request) {
        List<SnmpProxyRow> matches = getMatches(request);
        boolean allOK = true;
        for (SnmpProxyRow item : matches) {
            OctetString outParam = item.getSnmpProxyMultipleTargetOut();
            Set<OctetString> tags = SnmpTagList.getTags(outParam);
            if (logger.isDebugEnabled()) {
                logger.debug("Proxy multiple targets out with tags " + tags);
            }
            for (OctetString tag : tags) {
                Collection<SnmpTargetAddrEntryRow> targets = this.targetMIB.getTargetAddrRowsForTag(tag);
                for (SnmpTargetAddrEntryRow targetRow : targets) {
                    Target<A> target = targetRow.getTarget(request.getContextEngineID(), request.getContext(),
                            request.getCommandEvent().getStateReference().getAddress());
                    if (target != null) {
                        try {
                            PDU reqPDU = request.getCommandEvent().getPDU();
                            PDU pdu = DefaultPDUFactory.createPDU(target, reqPDU.getType());
    ...
    protected synchronized void fireIncrementCounter(CounterEvent event) {
