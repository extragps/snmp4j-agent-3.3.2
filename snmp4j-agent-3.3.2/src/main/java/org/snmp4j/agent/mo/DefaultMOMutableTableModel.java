/*_############################################################################
  _## 
  _##  SNMP4J-Agent 3 - DefaultMOMutableTableModel.java  
  _## 
...
public class DefaultMOMutableTableModel<R extends MOTableRow> extends DefaultMOTableModel<R>
     ...
     *
     * @return the possibly empty lexicographically ordered <code>List</code> of rows of this table model in the
     * specified index range. Modifications to the list will not affect the underlying table model, although
     * modifications to the row elements will.
     */
    public synchronized List<MOTableRow> getRows(OID lowerBound, OID upperBoundEx) {
        return new ArrayList<>(getView(lowerBound, upperBoundEx).values());
    }
    ...
    public synchronized List<R> clear(MOTableRowFilter<R> filter) {
        List<R> removedRows = new ArrayList<>();
    ...
    public synchronized void addMOTableModelListener(MOTableModelListener l) {
        if (moTableModelListeners == null) {
            moTableModelListeners = new ArrayList<>(2);
        }
    ...
    protected void fireTableModelChanged(MOTableModelEvent event) {
        final List<MOTableModelListener> listeners = moTableModelListeners;
        if (listeners != null) {
